package org.peacepact.app.web.rest;
...
import org.peacepact.app.config.EmbeddedKafka;
import org.peacepact.app.config.KafkaSseConsumer;
import org.peacepact.app.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class JhipsterPeacepactApplicationKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jhipster-peacepact-application-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jhipster-peacepact-application-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/jhipster-peacepact-application-kafka/unregister"));
                return;
            }
        }
